...
# this function will separate values with semi colons and give them new rows,
# then sum duplicate entries
df_to_single_protein <- function(df, Genelist) {
  df$uid <- as.character(seq_len(nrow(df)))
  build <- data.frame()
  for (i in seq_along(df[, Genelist])) {
    brick <- data.frame()
  ...
  separated_with_uid <- separated_with_uid[!duplicated(
    separated_with_uid[, Genelist]
  ), ]
...
ATG$LFQ <- sapply(
  ATG$Intensity_ES_vs_FM_ATG9A.Immunoisolation_intensity_filtered,
  function(df) {
    10^df
  }
)
names(ATG)[names(ATG) == "Gene_names"] <- "Gene.names"
ATG$FC10 <- ATG$Intensity_ES_vs_FM_ATG9A.Immunoisolation_intensity_filtered -
  ATG$Intensity_Mix_CTRL.IgM.Immunoisolation
...
# Separates values with semicolons and gives them new rows,
# then sums duplicate entries
