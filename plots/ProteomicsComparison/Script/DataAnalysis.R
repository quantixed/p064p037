...
# Read in cleaned data files. Should have a col of gene names, and LFQ values (or other quantitative measure)
...
ggsave("Output/Plots/INVvsSLMV_Plot.pdf", INVvsSLMV_Plot, width = 8, height = 8,bg = "white", units = "cm")
...
ggsave("Output/Plots/INVvsATG_Plot.pdf", INVvsATG_Plot, width = 8, height = 8, bg = "white", units = "cm")
...
ggsave("Output/Plots/ATGvsSLMV_Plot.pdf", ATGvsSLMV_Plot, width = 8, height = 8, bg = "white", units = "cm")
...
make_euler_2 <- function(A, B, AB, names = c("A", "B")) {
  ...
  ggsave(paste0("Output/Plots/",name,".pdf"), p, width = 3.5, height = 3.5, units = "cm")
}

make_euler_2(A = nrow(INV), B = length(unique(SLMV$SLMV_uid)), AB = length(unique(IS$SLMV_uid)), names = c("INV", "SLMV"))
make_euler_2(A = nrow(INV), B = length(unique(ATG$ATG_uid)), AB = length(unique(IA$ATG_uid)), names = c("INV", "ATG9"))

make_euler_3 <- function(A, B, C, AB, BC, AC, ABC, names = c("A", "B", "C")) {
  set1 <- c(A = A - AB - AC + ABC,
            B = B - AB - BC + ABC,
            C = C - BC - AC + ABC,
            "A&B" = AB - ABC,
            "A&C" = AC - ABC,
  ...
  p <- plot(fit1, quantities = TRUE)
  name <- paste(names, collapse = "_vs_")
  ggsave(paste0("Output/Plots/",name,".pdf"), p, width = 4, height = 4, units = "cm")
}

make_euler_3(A = nrow(INV),
             ...
             ABC = min(length(unique(ISA$ATG_uid)), length(unique(ISA$SLMV_uid))),
