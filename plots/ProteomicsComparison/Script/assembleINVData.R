...
# load the list of enriched proteins to allow us to select the proteins of interest
# this is the Igor-style output from DM103104
enriched_proteins <- read.delim("Data/combined_proteome.txt", header = TRUE)
# now load the data (not ranked table) output from Igor for DM and PE experiments
...
# order by descending productWave and then make sure shortnames and PIDs are unique
...
temp <-  merge(enriched_proteins, dm_df, by.x = "so_PID", by.y = "PID", all.x = TRUE, sort = FALSE)
# add the meanCond1 column to original enriched_proteins data frame
enriched_proteins$dm_int <- 2^temp$meanCond1
temp <-  merge(enriched_proteins, pe_df, by.x = "so_PID", by.y = "PID", all.x = TRUE, sort = FALSE)
...
enriched_proteins$intensity <- ifelse(is.na(enriched_proteins$dm_int),
                                      ...
                                      ifelse(enriched_proteins$dm_int > enriched_proteins$pe_int, enriched_proteins$dm_int, enriched_proteins$pe_int))
# now save the data frame to Output/Data/
write.table(enriched_proteins, file = "Output/Data/combined_inv_proteome_with_intensities.txt", sep = "\t", quote = FALSE, row.names = FALSE)
