...
# get a list of INV matches - this is needed because after collapse of ATG,
# we might not have the INV names anymore
...
# merge to add column which has the labelname of any uid that
# is in the INV dataset
ATG <- merge(ATG, INV_matches, by = "uid", all.x = TRUE)

ATG %>%
  ggplot(aes(x = logFC_ES_vs_FM_ATG9A.Immunoisolation_intensity_filtered,
             y = LFQ, shape = cat, colour = cat)) +
  geom_vline(xintercept = 0, linetype = "dashed", colour = "grey") +
  geom_point(size = 1) +
  scale_color_manual(values = c("ATG" = "#7f7f7f7f", "INV" = "#ab66f0")) +
  scale_shape_manual(values = c("ATG" = 1, "INV" = 16)) +
  geom_text_repel(data = subset(ATG, cat == "INV"), aes(label = labelnames),
                  size = 1.5, max.overlaps = 10, box.padding = 0.2) +
...
ggsave("Output/Plots/ATG_INV_label.pdf",
       width = 8, height = 8, bg = "white", units = "cm")
