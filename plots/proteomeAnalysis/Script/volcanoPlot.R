...
toMatch <- c("^TPD5","^RAB","^VAMP","ATG9","SCAMP","DAGLB","IGF2R","PI4K2A","SERINC","SYNGR", "SYPL", "^ITG", "^SLC2A1")
pois <- unique(grep(paste(toMatch,collapse="|"), pe$uSHORTNAME, value = TRUE))
# label the proteins of interest
pe$interest <- ifelse(pe$so_colorWave == 3 & pe$uSHORTNAME %in% pois, pe$uSHORTNAME, "")
...
p1 <- ggplot(pe, aes(x = log2(so_ratioWave), y = -log10(so_allTWave))) +
  ...
  scale_y_continuous(limits = c(0,20), breaks = seq(0,20,2), labels = vp_label) +
  xlab(bquote(INV~-~Control~(Log[2]))) +
  ylab("P-value") +
  theme_classic(8) +
  theme(legend.position = "none")

ggsave("Output/Plots/vp_pe.pdf", p1, width = 48, height = 80, units = "mm", family = "Helvetica")
...
ggsave("Output/Plots/vp_pe2.pdf", p2, width = 80, height = 80, units = "mm", family = "Helvetica")
