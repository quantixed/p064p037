...
retrieveUniprotInfo <- function(x) {
  files <- list(ids = paste0(x, collapse=", "),
                from = "Gene_Name",
  ...
  if (isJobReady(submission[["jobId"]])) {
    ...
    r <- GET(url = url, accept_json())
    resultsTable <- read.table(text = content(r), sep = "\t", header = TRUE, fill = TRUE, quote="")
    
    while (exists("link", where = r$headers)) {
      url <- str_extract(r$headers$link[1], "(?<=<).*(?=>)")
      r <- GET(url = url, accept_json())
      resultsTable <- rbind(resultsTable, read.table(text = content(r), sep = "\t", header = TRUE, fill = TRUE, quote = ""))
      # print(length(resultsTable$Protein.names))
    }
  }
  return(resultsTable)
}

incidence_of_tm <- function(df1, df2, df1label = "df1", df2label = "df2", tmopt = FALSE, lab = "") {
  ...
  plotdf <- propsdf %>% pivot_longer(cols = c(df1label,df2label),
                                     names_to = "group",
                                     values_to = "freq")
  ggplot(plotdf, aes(x = tms, y = freq, fill = group)) +
    ...
    scale_x_continuous(limits = c(0.5,max(plotdf$tms) + 0.5), breaks = seq(from = 1, to = max(plotdf$tms), by = 2)) +
    theme_cowplot(9) +
...
posthoc <- function(df) {
  ...
  v <- NULL
  for (i in 1:nc) {
    n <- rbind(c(m[1,i], m_sum[1] - m[1,i]),
               c(m[2,i], m_sum[2] - m[2,i]))
    v <- rbind(v, chisq.test(n)$p.value)
  }
  result <- data.frame(tms = mnames,
                       p.value = v)
  alpha <- 0.05 / (nc - 1)
  ...
  result$bh <- ifelse(result$p.value < 1 & result$p.value < (0.05 / (nc - (row(result) - 1))),"reject","accept")[,1]
  result <- result[order(as.numeric(result$tms), decreasing = FALSE), ]
  return(result)
}

incidence_of_secreted <- function(df1, df2, df1label = "df1", df2label = "df2") {
  # incidence of transmembrane domain-containing proteins
...
process_output_dataset <- function(path1, path2 = NULL, label) {
  if(is.null(path2)) {
    ...
    if(length(uhits) != length(uhits %in% uuniverse)) {
      print("Number of unique hits does not match the number found in universe\n")
    }
  }
  ...
  df <- merge(universe, uni_results, by.x = "so_SHORTNAME", by.y = "From", all.x = TRUE, all.y = FALSE)
  ...
  # at this point we can have duplicated entries where one has 0 TM domains and the other has more
  # we will reject the duplicate(s) with the lowest value, because duplicated takes the first value
  ...
  cat(paste0("*** ", label, " ***\n"))
  incidence_of_tm(df1 = df, df2 = hits, df1label = "IP", df2label = "INV", lab = label)
  incidence_of_secreted(df1 = df, df2 = hits, df1label = "IP", df2label = "INV")
  
  # save data note that the fold change and p-value relates to universe so only use output for Uniprot data
...
process_output_dataset(path1 = "Data/rankTable_INVvsControlPE.txt", label = "PE")
process_output_dataset(path1 = "Data/rankTable_WTvsControlDM.txt", label = "DM")
process_output_dataset(path1 = "Data/rankTable_INVvsControlBoth.txt", label = "both")

process_output_dataset(path1 = "Data/combined_proteome.txt", path2 = "Data/universe_proteome.txt", label = "combined")
sink()
