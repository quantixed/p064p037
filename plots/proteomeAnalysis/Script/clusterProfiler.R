...
make_ego_plots <- function(obj,s) {
  ...
  p1 <- goplot(obj)
  ggsave(filename = paste0("Output/Plots/","cp_go_",s,".pdf"), plot = p1, width = 20, height = 20, units = "cm", dpi = 300, bg = "white")
  p2 <- dotplot(obj, showCategory = l[1:15], font.size = 12)
  p2 <- p2 + scale_size_area(max_size = 6)
  ggsave(filename = paste0("Output/Plots/","cp_dot_",s,".pdf"), plot = p2, width = 16, height = 12, units = "cm")
  p3 <- cnetplot(obj, showCategory = l[1:10], cex.params = list(category_label = 0.6, gene_label = 0.4), max.overlaps = 12)
  ggsave(filename = paste0("Output/Plots/","cp_cnet_",s,".pdf"), plot = p3, width = 25, height = 19, units = "cm", dpi = 300, bg = "white")
}
...
cnetplot(kk, showCategory = 10, cex.params = list(category_label = 0.6, gene_label = 0.4), max.overlaps = 12)
