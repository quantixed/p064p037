...
list_all <- list(DM = dm_df$so_SHORTNAME,
...
# the order should be by fold change, RE proteins at the end, with TPD52L2 exempted

# remove RE only
all_int_all <- all_int_all[all_int_all$int != "RE",]
# first combine with dm_df
combine_df <- merge(all_int_all, dm_df, by.x = "protein", by.y = "so_SHORTNAME", all.x = TRUE, sort = FALSE)
...
df2 <- merge(missing_df, pe_df, by.x = "protein", by.y = "so_SHORTNAME", all.x = TRUE, sort = FALSE)
...
# keep <- c("protein","int","so_NAME","so_PID","so_productWave","so_colorWave","so_allTWave","so_ratioWave","so_keyW")
