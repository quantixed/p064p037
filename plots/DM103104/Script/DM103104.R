...
keep <- c(
  "so_SHORTNAME", "so_NAME.x", "so_PID.x", "so_productWave.x",
  "so_colorWave.x", "so_allTWave.x", "so_ratioWave.x",
...
names(all_df) <- c(
  "SHORTNAME", "NAME", "PID", "productWave_wt_ct", "colorWave_wt_ct",
  "allTWave_wt_ct", "ratioWave_wt_ct",
...
coolors <- rev(c(
  "#4122a6", "#712771", "#f11919",
  "#2c5270", "#808080", "#ae7d3c",
  "#14c114", "#95a83a", "#f1cc19"
))

# bit0 is significant in 1st set, bit1 is significant in 2nd
all_df$pcat1 <- ifelse(all_df$allTWave_wt_ct < 0.05, 1, 0) +
  ifelse(all_df$allTWave_wt_re < 0.05, 2, 0)
...
all_df$label1 <- ifelse((all_df$pcat1 == 3) & (all_df$encat1 == 8),
                        all_df$uSHORTNAME, "")


p1 <- ggplot(all_df, aes(x = log2(ratioWave_wt_ct), y = log2(ratioWave_wt_re),
                         label = label1)) +
  ...
  coord_fixed() +
  labs(x = bquote(WT ~ -~Control ~ (Log[2])),
       y = bquote(WT ~ -~R159E ~ (Log[2]))) +
  theme_classic(8) +
...
# bit0 is significant in 1st set, bit1 is significant in 2nd
all_df$pcat2 <- ifelse(all_df$allTWave_wt_ct < 0.05, 1, 0) +
  ifelse(all_df$allTWave_re_ct < 0.05, 2, 0)
# now we have 3 levels (up, mid, down) for two coordinates, 9 levels, 0-8.
...
# labels for only the proteins of interest
all_df$label2 <- ifelse((all_df$pcat2 == 3) & (all_df$encat2 == 8),
                        all_df$uSHORTNAME, "")


p2 <- ggplot(all_df, aes(x = log2(ratioWave_wt_ct),
                         y = log2(ratioWave_re_ct), label = label2)) +
  geom_vline(xintercept = 0, linetype = "dashed", colour = "grey") +
  geom_hline(yintercept = 0, linetype = "dashed", colour = "grey") +
...
make_volcano_plot <- function(df, vs, aa, bb, poi = NULL) {
  ...
  if (is.null(poi)) {
    ...
    df$interest <- ifelse(df[[zw]] == 3 & df$uSHORTNAME %in% poi,
                          df$uSHORTNAME, "")
  }
  ...
  p <- ggplot(df, aes(x = log2(!!xw), y = -log10(!!yw), label = interest)) +
    ...
    theme_classic(8) +
    theme(legend.position = "none")
  ggsave(paste0("Output/Plots/vp_", vs, ".pdf"), p,
         width = 48, height = 80, units = "mm", family = "Helvetica")
  return(p)
}

toMatch <- c("^TPD5", "^RAB", "^VAMP", "ATG9", "SCAMP", "DAGLB",
             "IGF2R", "PI4K2A", "SERINC", "SYNGR", "SYPL")
pois <- unique(grep(paste(toMatch, collapse = "|"),
                    all_df$uSHORTNAME, value = TRUE))
